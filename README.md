...
## Overview 
*a.k.a everything we know about the project and how we did it.*
...
> [!NOTE]  
> Heredocs are an essential feature of any shell. They allow you to input multi-line text directly in the terminal.
...
> [!TIP]
> Use single quotes when you want to prevent variable expansion, and double quotes when you need to preserve spaces but still want variables to be expanded.
...
> [!IMPORTANT]  
> Changes to environment variables using export and unset only affect the current shell session and its child processes.
...
> [!CAUTION]
> Always check the exit status (\$?) after critical commands to ensure they executed successfully.
...
> [!WARNING]  
> Signal handling is critical for proper shell operation. Incorrect signal handling can lead to orphaned processes or unresponsive shell behavior.
...
1. **Read Input**: The shell displays the prompt (Minishell> ) and waits for the user to type a command line. We need to use readline() from the Readline library to handle this. This provides editing features and a command history. After the user presses Enter, readline returns the input line as a string.
...
2. Before going to the next step I want to mention that readline can cause some memory leaks as said in the project description. So don´t worry if you see some leaks in your project. We tried to avoid them as much as possible but we couldn´t get rid of all of them. We used valgrind to check for memory leaks and we got some of them but not all.

3. **Input Validation**: Before heavy processing, Minishell does some quick validations:
...
4. **Lexical Analysis (Tokenization)**: The first major processing step is to tokenize the input string. The Lexer module goes through the input character by character and groups characters into meaningful tokens:
...
5. The lexer basically breaks the input into a linked list of t_token structures. Each token has a type (WORD, PIPE, REDIR_IN, REDIR_OUT, HEREDOC, etc.) and a string value. During this phase, we also classify >> as a single token of type "append redirection" and << as HEREDOC. The tokenization process also identifies unclosed quotes or invalid sequences. For instance, if there's a quote without a matching closing quote, the lexer can flag an error. Or if special characters like ; or & appear (which our Minishell doesn´t support as operators), it can mark that as a syntax error. If any of this issues are found, Minishell will print an error message (e.g., "Syntax error: unclosed quotes") and skip execution for this line, returning the prompt for a new input.

6. After tokenizing, Minishell performs a syntax check on the token list to catch errors like:
...
10. After parsing, we have a linked list minishell->commands of commands ready to execute. Each command contains:
...
13. **Pipeline Forking**: Executing a pipeline of N commands involves N processes (for N commands) that run concurrently, connected by pipes. Minishell´s executor handles this as follows:
...
14. **Cleanup**: Once a command (or pipeline) has been executed, Minishell performs cleanup before returning to the prompt:
...
15. The shell then goes back to step 1, printing a new prompt and waiting for the next command. This loop continues until an exit condition is met (the user types exit or sends EOF).
...
Feel free to reach out to either contributor via GitHub for any questions, suggestions, or opportunities. We are proud of Minishell and happy to discuss its implementation or potential improvements. Thank you for reading about our project!
...
* The shell does not support command history or command completion (like Bash's tab completion). It only supports basic readline functionality.

* Some error messages may not be as descriptive as in Bash, but we tried to provide useful feedback for common errors.
